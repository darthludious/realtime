...
from .flowise_ai import FlowiseAI
...
flowise_ai = FlowiseAI()
computer_control = ComputerControl()

@timeit_decorator

async def query_ai_agent(agent_type: str, question: str, context: Optional[Dict[str, Any]] = None) -> dict:

    """

    Query a specific AI agent through Flowise named Jarvis, who is connected to multiple apis and a team of specialized agents. Send complex tasks to be done and interpret the results.

    """

    try:

        result = await flowise_ai.query_agent(agent_type, question, context)
        ...
        return {
            ...
            "message": f"Failed to query AI agent: {str(e)}"
...
# Computer Control Response Models
...
class MouseControlResponse(BaseModel):
    action: MouseAction
    x: Optional[int] = None
    y: Optional[int] = None
...
class KeyboardControlResponse(BaseModel):
...
async def control_mouse(prompt: str) -> dict:
    """Control mouse actions based on the user's prompt."""
    try:
        response = await structured_output_prompt(prompt, MouseControlResponse)
        
        if response.action == MouseAction.MOVE:
            if response.x is not None and response.y is not None:
                computer_control.mouse_move(response.x, response.y, duration=response.duration or 0.2)
            elif response.target_image:
                location = computer_control.find_on_screen(response.target_image)
                if location:
                    computer_control.mouse_move(*location)
                else:
                    return {"status": "error", "message": f"Could not find target image on screen"}
        elif response.action == MouseAction.CLICK:
            computer_control.mouse_click(button='left')
        elif response.action == MouseAction.RIGHT_CLICK:
            computer_control.mouse_click(button='right')
        elif response.action == MouseAction.DOUBLE_CLICK:
            computer_control.mouse_click(button='left')
            computer_control.mouse_click(button='left')
        elif response.action == MouseAction.DRAG:
            if all(x is not None for x in [response.x, response.y]):
                current_pos = pyautogui.position()
                computer_control.mouse_drag(
                    current_pos.x, current_pos.y,
                    response.x, response.y,
                    duration=response.duration or 0.2
                )
            
        return {"status": "success", "message": f"Successfully performed mouse action: {response.action}"}
    except Exception as e:
        return {"status": "error", "message": f"Failed to control mouse: {str(e)}"}

@timeit_decorator
async def control_keyboard(prompt: str) -> dict:
    """Control keyboard actions based on the user's prompt."""
    try:
        response = await structured_output_prompt(prompt, KeyboardControlResponse)
        
        if response.action == KeyboardAction.TYPE:
            if response.text:
                computer_control.type_text(response.text)
        elif response.action == KeyboardAction.PRESS:
            if response.key:
                computer_control.press_key(response.key)
        elif response.action == KeyboardAction.HOTKEY:
            if response.keys:
                computer_control.hotkey(*response.keys)
                
        return {"status": "success", "message": f"Successfully performed keyboard action: {response.action}"}
    except Exception as e:
        return {"status": "error", "message": f"Failed to control keyboard: {str(e)}"}

@timeit_decorator
async def take_screenshot(prompt: str = "") -> dict:
    ...
    try:
        ...
        screenshot = computer_control.screenshot()
...
@timeit_decorator
async def extract_text_from_screen(prompt: str = "") -> dict:
    ...
    try:
        ...
        text = computer_control.get_text_from_image(screenshot)
        
        # Save both the screenshot and extracted text
        scratch_pad_dir = os.getenv("SCRATCH_PAD_DIR", "./scratchpad")
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # Save screenshot
        screenshot_path = os.path.join(scratch_pad_dir, f"ocr_screenshot_{timestamp}.png")
        screenshot.save(screenshot_path)
        
        # Save extracted text
        text_path = os.path.join(scratch_pad_dir, f"ocr_text_{timestamp}.txt")
        with open(text_path, 'w', encoding='utf-8') as f:
            f.write(text)
        
        return {
            "status": "success",
            "message": f"Text extracted and saved to {text_path}",
            "text": text,
...
@timeit_decorator
async def control_window(prompt: str) -> dict:
    """Control window actions based on the user's prompt."""
    try:
        response = await structured_output_prompt(prompt, WindowControlResponse)
        
        # Find the target window
        windows = computer_control.get_window_by_title(response.window_title)
        if not windows:
            return {"status": "error", "message": f"Could not find window with title: {response.window_title}"}
        
        window = windows[0]  # Use the first matching window
        
        if response.action == WindowAction.FOCUS:
            computer_control.focus_window(window)
        elif response.action == WindowAction.MOVE:
            computer_control.move_window(window, response.x, response.y)
        elif response.action == WindowAction.RESIZE:
            computer_control.resize_window(window, response.width, response.height)
        elif response.action == WindowAction.MINIMIZE:
            window.minimize()
        elif response.action == WindowAction.MAXIMIZE:
            window.maximize()
        elif response.action == WindowAction.RESTORE:
            window.restore()
        elif response.action == WindowAction.CLOSE:
            window.close()

        return {"status": "success", "message": f"Successfully performed window action: {response.action}"}
    except Exception as e:
        return {"status": "error", "message": f"Failed to control window: {str(e)}"}
@timeit_decorator
...
tools = [

    {
        ...
        "description": "Queries a team of AI agents through Flowise with a task and optional context.",
